...
const BillView = () => {
  ...
  const handleApprovalAction = async (action) => {
    ...
    try {
      const endpoint =
        action === "accept"
          ? `/form/bill/accept/${selectedForm.id}`
          : `/form/bill/reject/${selectedForm.id}`;
      ...
      if (response) {
        toast.success("bill form Reviewed Successfully");
      }
      setRemarks("");
    } catch (error) {
      setRemarks("");
      ...
      <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-gray-800">
            {modalType === "accept" ? "Accept bill form" : "Reject bill form"}
          </h2>
          <button
        ...
        <div className="bg-yellow-50 border border-yellow-200 p-3 rounded-lg mb-4 flex items-center">
          ...
          <p className="text-yellow-800 text-sm">
            {modalType === "accept"
              ? "You are about to approve this bill form."
              : "You are about to reject this bill form."}
          </p>
        </div>
  ...
  const renderProjectRows = () => {
      ...
      return data.map((project) => (
        ...
        >
          ...
          <td className="px-6 py-4">
            {project.bill === -1 || project.bill === 0 ? (
              <button
                onClick={() => navigate(`/create/bill/${project.id}`)}
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => navigate(`/view/bill-supply/${project.id}`)}
    ...
    if (role === "HOD") {
      ...
      const pendingForms = data.filter((project) => project.bill === 1);
      const acceptedForms = data.filter((project) =>
        [3, 4, 5].includes(project.bill)
      );
      const rejectedForms = data.filter((project) => project.bill === 2);
    ...
    if (role === "DEAN") {
      ...
      const pendingForms = data.filter((project) => project.bill === 3);
      const acceptedForms = data.filter((project) => project.bill === 5);
      const rejectedForms = data.filter((project) => project.bill === 4);
    ...
    <div className="bg-gray-50 min-h-screen pb-12">
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 pt-4">
          <div>
            <p className="text-gray-600 mt-1">Manage your project bill forms</p>
          </div>
        </div>
