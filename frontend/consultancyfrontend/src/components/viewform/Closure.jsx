...
const ClosureView = () => {
  ...
  const handleApprovalAction = async (action) => {
    ...
    try {
      const endpoint =
        action === "accept"
          ? `/form/closure/accept/${selectedForm.id}`
          : `/form/closure/reject/${selectedForm.id}`;
      ...
      if (response) {
        toast.success("Closure form Reviewed Successfully");
      }
      setRemarks("");
    } catch (error) {
      setRemarks("");
      ...
      <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-2xl">
        ...
        <div className="bg-yellow-50 border border-yellow-200 p-3 rounded-lg mb-4 flex items-center">
          ...
          <p className="text-yellow-800 text-sm">
            {modalType === "accept"
              ? "You are about to approve this closure form."
              : "You are about to reject this closure form."}
          </p>
        </div>
  ...
  const renderProjectRows = () => {
      ...
      return data.map((project) => (
        ...
        >
          ...
          <td className="px-6 py-4">
            {project.closure === -1 || project.closure === 0 ? (
          ...
          <td className="px-6 py-4">
            {project.closure === -1 || project.closure === 0 ? (
              <button
                onClick={() => navigate(`/create/closure/${project.id}`)}
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => navigate(`/view/closure/${project.id}`)}
    ...
    if (role === "HOD") {
      ...
      const pendingForms = data.filter((project) => project.closure === 1);
      const acceptedForms = data.filter((project) =>
        [3, 4, 5].includes(project.closure)
      );
      const rejectedForms = data.filter((project) => project.closure === 2);
    ...
    if (role === "DEAN") {
      ...
      const pendingForms = data.filter((project) => project.closure === 3);
      const acceptedForms = data.filter((project) => project.closure === 5);
      const rejectedForms = data.filter((project) => project.closure === 4);
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 pt-4">
          <div>
            <p className="text-gray-600 mt-1">
              Manage your project closure forms
            </p>
          </div>
        </div>
